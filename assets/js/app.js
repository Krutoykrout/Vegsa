...
const carUrl = (car) => page(`cars/${car.id}.html`);
const waLink = (text) => `${CONFIG.whatsapp}?text=${encodeURIComponent(text)}`;

function setCommon(){
  qsa('[data-phone-text]').forEach(el => el.textContent = CONFIG.phoneDisplay || '8 967 050-20-20');
  qsa('[data-phone-href]').forEach(el => el.href = `tel:${CONFIG.phoneHref || '+79670502020'}`);
...
function specsMini(car){
  const items = [['Кузов', car.bodyType], ['Топливо', car.fuel], ['Коробка', car.transmission], ['Привод', car.drive], ['Год', car.year]];
  return `<div class="spec-box spec-box--mini" aria-label="Краткие характеристики">${items.map(([k,v]) => `<div class="spec-row"><span>${k}</span><b>${v}</b></div>`).join('')}</div>`;
}
function carCard(car, index=0){
  return `<article class="car-card reveal" style="--delay:${Math.min(index, 9) * 45}ms" data-car-card data-car-id="${car.id}">
...
function renderPopular(cars){
  ...
  const ids = ['lada-granta','lada-vesta','lada-largus','moskvich-3','geely-emgrand','voyah-free'];
...
function initCatalog(cars){
  ...
  function render(resetVisible=false){
    ...
    grid.innerHTML = visible.length ? visible.map(carCard).join('') : `<div class="empty-state">По таким параметрам машины не нашлись. Попробуйте убрать один из фильтров.</div>`;
...
document.addEventListener('DOMContentLoaded', () => { setCommon(); renderPopular(CARS); initCatalog(CARS); initGallery(); initPhotoLightbox(); observeReveal(); });
